...
void drawWall(int x1, int x2, int b1, int b2, int t1, int t2, int c, int s){
	...
	for (x = x1; x < x2; x++){
		...
		//pixel(x, y1, 0); // bottom line
		//pixel(x, y2, 0); // top line
...
void draw3D(){
	...
	// Order sectors by distance (bubble sort)
	...
	for (s= 0; s < numSect; s++){
		...
		for(loop=0; loop < 2; loop++){		
			for(w=S[s].ws; w<S[s].we; w++){	
				...
				//int x1=40-P.x; int y1=10-P.y;
				//int x2=40-P.x; int y2=290-P.y;
				int x1=W[w].x1-P.x; int y1=W[w].y1-P.y;
				int x2=W[w].x2-P.x; int y2=W[w].y2-P.y;
				
				// Swap for surface / Draw back walls:
